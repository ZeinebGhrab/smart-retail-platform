...
class RetrieverTests:
    ...
    def test_csv_structure(self) -> bool:
        ...
        try:
            ...
            # Colonnes attendues (colonnes réelles du CSV shoppingclub_2025_2026)
            expected_cols = {'camera', 'datetime', 'gender', 'age', 'Visits'}
...
class GenerationTests:
    ...
    def test_faithfulness_threshold(self) -> bool:
        ...
        try:
            ...
            faithfulness = (
                report.get("aggregate", {}).get("generation", {}).get("faithfulness")
                or report.get("generation_metrics", {}).get("faithfulness", 0)
            )
    ...
    def test_answer_relevancy_threshold(self) -> bool:
        ...
        try:
            ...
            relevancy = (
                report.get("aggregate", {}).get("generation", {}).get("answer_relevancy")
                or report.get("generation_metrics", {}).get("answer_relevancy", 0)
            )
...
class PredictionTests:
    ...
    def test_model_prediction(self) -> bool:
        """Teste une prédiction simple avec les features réelles du modèle LightGBM"""
        try:
            with open(Config.MODEL_PATH, "rb") as f:
                model = pickle.load(f)

            # Features exactes attendues par le modèle entraîné
            MODEL_FEATURES = [
                "day_of_week", "is_weekend", "month", "week_of_year", "quarter",
                "is_pre_holiday", "is_post_holiday", "is_school_holiday",
                "pre_holiday_name_enc", "days_to_next_holiday", "days_since_last_holiday",
                "temp_max_c", "temp_min_c", "temp_range_c", "precipitation_mm",
                "wind_kmh", "humidity_pct", "weather_comfort_score",
                "is_rainy", "heat_stress", "weather_category_enc",
            ]

            # Échantillon synthétique avec des valeurs plausibles
            sample_data = {
                "day_of_week": [2], "is_weekend": [0], "month": [6],
                "week_of_year": [26], "quarter": [2],
                "is_pre_holiday": [0], "is_post_holiday": [0], "is_school_holiday": [0],
                "pre_holiday_name_enc": [0], "days_to_next_holiday": [5],
                "days_since_last_holiday": [10],
                "temp_max_c": [28.0], "temp_min_c": [18.0], "temp_range_c": [10.0],
                "precipitation_mm": [0.0], "wind_kmh": [15.0], "humidity_pct": [55.0],
                "weather_comfort_score": [0.75], "is_rainy": [0],
                "heat_stress": [0], "weather_category_enc": [1],
            }
            X = pd.DataFrame(sample_data)[MODEL_FEATURES]
            predictions = model.predict(X)

            valid = len(predictions) == 1
            log_test("Model can make predictions", valid, f"prediction={predictions[0]:.1f} visitors")
