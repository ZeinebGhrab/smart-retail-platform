...
const Dashboard: React.FC = () => {
  ...
  const handleNewPrediction = useCallback((data: PredictionData) => {
    // ✅ Vérifie que data.prediction existe avant de l'utiliser
    if (!data || !data.prediction) return;

    setPrediction(data);

    setNotifications(prev => prev.map(n =>
      n.isPrediction ? { ...n, msg: data.message ?? '', time: timeLabel(), unread: true } : n
    ));

    setAlerts(prev => prev.map(a =>
      a.id === 3 ? { ...a, subtitle: `Rapport du ${data.date} · Llama 3.2`, time: timeLabel(), unread: true } : a
    ));

    setBadgeCount(prev => prev + 1);

    const shortMsg = data.message ? data.message.substring(0, 90) + '…' : 'Nouvelle prédiction disponible.';
    ...
    <IonPage className="db-page">
      ...
      <div className="db-header">
        ...
        <div className="db-tab-row">
          {NAV_TABS.map(tab => (
            <button key={tab} className={`db-tab-pill ${activeTab === tab ? 'active' : ''}`} onClick={() => setActiveTab(tab)}>
              {tab}
            </button>
          ))}
        </div>
      ...
      <IonContent className="db-content">
        ...
        <div className="db-kpi-grid">
          ...
          <KpiCard icon="alert-triangle" label="Alertes actives"    value={String(badgeCount)} delta={`${alerts.filter(a => a.unread).length} non traitées`} deltaNeg/>
        </div>
        ...
        <div className="db-chart-card">
          <div className="db-chart-header">
            ...
            <button className="db-export-btn"><span className="ti ti-upload" aria-hidden="true"/> Export</button>
          </div>
        ...
        {/* ✅ Prediction card — guards contre prediction.prediction undefined */}
      ...
      <IonFooter className="db-bottom-nav">
        ...
        <button className="db-nav-item" onClick={() => setNotifOpen(true)}>
      ...
      {/* Notification Panel */}
      {notifOpen && (
        <div className="db-notif-overlay" onClick={e => e.target === e.currentTarget && setNotifOpen(false)}>
